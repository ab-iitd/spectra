    ...
    if min_num_params > 0:
        ...
        for layer_class in fsdp_transformer_layer_cls_to_wrap:
            ...
            if transformer_cls is None:
                # If we can't find the specified class, try to auto-detect for InternVL and other models
                print(f"Warning: Could not find layer class '{layer_class}' in model. Attempting auto-detection...")
                
                # Auto-detect transformer layer classes
                detected_classes = set()
                for name, submodule in module.named_modules():
                    class_name = submodule.__class__.__name__
                    # Common decoder layer patterns
                    if 'DecoderLayer' in class_name or 'TransformerBlock' in class_name or 'Block' in class_name:
                        detected_classes.add(submodule.__class__)
                
                if detected_classes:
                    print(f"Auto-detected transformer layer classes: {[cls.__name__ for cls in detected_classes]}")
                    transformer_cls_to_wrap.update(detected_classes)
                else:
                    raise Exception(
                        f"Could not find the transformer layer class '{layer_class}' to wrap in the model. "
                        f"Available module types: {set(m.__class__.__name__ for _, m in module.named_modules())}"
                    )
            else:
