...
def process_image(image: dict | Image.Image) -> Image.Image:
    ...
    if not isinstance(image, dict):
        raise TypeError(f"process_image expects dict|PIL, got {type(image)}")
    ...
    # Common keys used across datasets:
    # - {"image": "img.png"}
    # - {"url": "file:///abs/path/img.png"}
    # - {"image_url": "..."}
    path = image.get("image") or image.get("url") or image.get("image_url")
    if isinstance(path, str):
        # local path without scheme
        if "://" not in path and os.path.exists(path):
            return Image.open(path).convert("RGB")
        # let qwen_vl_utils handle file:// / http(s):// etc
        image = dict(image)
        if "image" not in image:
            image["image"] = path
        return fetch_image(image).convert("RGB")
