    ...
    def execute_code(self, instance_id, code, timeout=30, language="python"):
        try:
            ...
            if data.get("success"):
                ...
                #msg = json.dumps({"success": "True", "message": msg})
                msg = json.dumps(data)
                logger.debug(f"Azure response for {instance_id}: {msg}")
                return ToolResponse(text=msg)
            else:
                return ToolResponse(text="Execution failed: " + json.dumps(data))
        except Exception as e:
            logger.warning(f"Error contacting Azure container: {e}")
            return ToolResponse(text=f"Exception during execution: {e}")
